...
! computes the energy at position at time t
 call compute_dist(npart,xyzt,newr1,newr2,newthe)
 do ista=1,nsta
   call db3val(newr2,newr1,newthe,idr1,idr2,idthe,tr2(ista,:),tr1(ista,:),tthe(ista,:),nr2,nr1,nthe,kr1,kr2,kt,energy(ista,:,:,:),val(ista),iflag,& 
 inbvx,inbvy,inbvz,iloy,iloz)
 if(iflag/=0) then
...
! computes the new position
 do i = 1, npart
   do j = 1, 3
    ...
    call compute_dist(npart,xyzdr,newr1,newr2,newthe)
 do ista=1,nsta
    ...
    inbvx,inbvy,inbvz,iloy,iloz)
 enddo
 !!   write(*,'(i4,i4,5(f15.5))')j,i,newr1,newr2,newthe,valdrp(fsta),val(fsta)
 ...
 do ista=1,nsta
    ...
    inbvx,inbvy,inbvz,iloy,iloz)
 enddo
!!    write(*,'(i4,i4,5(f15.5))')j,i,newr1,newr2,newthe,valdrm(fsta),val(fsta)
